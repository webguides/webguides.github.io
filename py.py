...
print(f"🔑 API: {'LOADED' if API_KEY else 'MISSING!!! STOPPING'} (len: {len(API_KEY) if API_KEY else 0})")
if not API_KEY:
    exit(1)
...
# FULL KEYWORDS - PASTE YOUR ENTIRE LIST HERE (from original script - 500+ items!)
keywords = [
...
print(f"📝 Keywords total: {len(keywords)} - First 3: {keywords[:3]}")

def load_used_keywords():
    used = set()
    if os.path.exists(USED_KEYWORDS_FILE):
        with open(USED_KEYWORDS_FILE, 'r') as f:
            used = {line.strip() for line in f if line.strip()}
        print(f"📋 Used keywords: {len(used)} loaded")
    return used

def save_used_keyword(keyword):
    with open(USED_KEYWORDS_FILE, 'a') as f:
        f.write(keyword + '\n')
    print(f"💾 Marked '{keyword}' as used")

def generate_blog_post(keyword):
    print(f"🤖 Calling Cerebras for: {keyword}")
    try:
        stream = client.chat.completions.create(
            messages=[{"role": "system", "content": f"Write a 800-1200 word blog post on '{keyword}'. Conversational, tips, stories, active voice."}],
            model="llama3.1-8b", stream=True, max_completion_tokens=4000, temperature=0.7
        )
        content = ""
        for chunk in stream:
            if chunk.choices[0].delta.content:
                content += chunk.choices[0].delta.content
        print(f"📄 Generated: {len(content)} chars for {keyword}")
        return content.strip()
    except Exception as e:
        print(f"💥 API ERROR on {keyword}: {e}")
        return ""
...
def main():
    ...
    saved_count = 0
    try_count = 0
    max_time = 600  # 10 min for debug

    for i, kw in enumerate(keywords, 1):
        if time.time() - start > max_time or saved_count >= 3:
            print(f"🛑 DEBUG STOP: Time up or 3 saved. Tries: {try_count}, Saved: {saved_count}")
            break
        if kw in used:
            print(f"⏭️ SKIP {i}: {kw} (used)")
            continue

        try_count += 1
        print(f"[{try_count}] TRYING: {kw}")
        content = generate_blog_post(kw)
        if content and len(content) > 500:
            save_post(kw, content)
            save_used_keyword(kw)
            saved_count += 1
            print(f"🎯 SUCCESS {saved_count}/3: {kw}")
        else:
            print(f"❌ FAIL {try_count}: {kw} (short/empty)")
        
        time.sleep(3)  # Slower for debug

    print(f"🏁 END: {saved_count} saved in {(time.time() - start)/60:.1f} min")
