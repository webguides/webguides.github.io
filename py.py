...
if not API_KEY:
    print("CEREBRAS_API_KEY not found! Check repo secrets.")
    exit(1)
...
# YOUR FULL KEYWORDS LIST - ADD ALL 500+ HERE
keywords = [
...
def generate_blog_post(keyword):
    try:
        stream = client.chat.completions.create(
            messages=[{
                "role": "system",
                "content": f"Write a 800-1200 word conversational blog post about '{keyword}'. Use personal stories, practical tips, active voice. Make it helpful and engaging."
            }],
            model="llama3.1-8b",
            stream=True,
...
def main():
    ...
    for i, keyword in enumerate(keywords, 1):
        if time.time() - start_time > max_runtime:
            print(f"1 HOUR LIMIT REACHED. Generated: {generated}")
            break

        if keyword in used:
            print(f"[{i}] SKIP: {keyword}")
            continue
        ...
        if content and len(content) > 500:
            ...
            generated += 1
            print(f"DONE: {keyword} ({len(content)} chars)")
        else:
            print(f"FAILED: {keyword} (too short or empty)")

        time.sleep(1)

    print(f"FINISHED: {generated} posts in {(time.time() - start_time)/60:.1f} min")
